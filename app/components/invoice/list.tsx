  ...
  const renderedList = list.map((item) => {
    ...
    if (props.layout === 'multi') {
      ...
      return (
        <tr key={invoice.id}>
          <td>
            <p><span className="text-black dark:text-slate-700">{t("number")}:</span>&nbsp;&nbsp;{invoice.number}</p>
            <p><span className="text-black dark:text-slate-700">{t("date")}:</span>&nbsp;&nbsp;{invoice.date}</p>
            <p><span className="text-black dark:text-slate-700">{t("amount")}:</span>&nbsp;&nbsp;{invoice.amount.toFixed(2)}</p>
            <p><span className="text-black dark:text-slate-700">{t("payment")}:</span>&nbsp;&nbsp;{t(invoice.payment)}</p>
            <p><span className="text-black dark:text-slate-700">{t("paid")}:</span>&nbsp;&nbsp;{invoice.paid ? t('yes') : t('no')}</p>
          </td>
        </tr>
      );
    }
